...
<div id="tsp-demo" style="text-align: center; margin: 20px 0;">
    <h3>🗺️ Interactive Travelling Salesman Demo: Click on the map to add cities, then see how many possible routes there are. Watch the complexity explode!
</h3>
    ...
    <div id="tsp-info" style="margin: 15px 0; font-family: monospace; font-size: 16px; background: #f8f9fa; padding: 15px; border-radius: 8px;">
        ...
        <div style="margin-top: 10px; color: #E74C3C;"><strong>⚠️ Complexity:</strong> <span id="complexity-warning">Add cities to see the challenge!</span></div>
    </div>
    <div style="margin: 10px 0; font-size: 14px; color: #7f8c8d;">
        💡 <strong>Tip:</strong> Click anywhere on the map to add cities at exact locations, or use "Add Random City" for quick setup!
    </div>
    <div style="margin: 5px 0; font-size: 12px; color: #95a5a6;">
        🎯 <strong>Interactive Mode:</strong> Click on the canvas to place cities exactly where you want them!
    </div>
</div>
```
...
## Time Complexity: When Mathematics Meets Reality ⏱️
...
def test_tsp_performance():
    ...
    for n_cities in range(3, 8):  # Stop at 7 cities (720 routes)
        ...
        if n_cities >= 10:
            print(f"   ⚠️  Warning: {n_cities} cities would take MUCH longer!")
            break
...
**Reality Check**: Professional delivery companies like UPS have trucks visiting 100+ stops per day. Our brute force algorithm would take longer than the age of the universe to solve this! 🤯
 -->
...
## Think Deeper: Environmental Consequences 🌍
...
**3. Future Transportation Challenges** 🔋
...
- 🛣️ **Shortest distance** (saves fuel, reduces costs)
- 🚦 **Avoiding traffic** (faster delivery, customer satisfaction)  
- ⚖️ **Serving all neighborhoods equally** (social equity, fairness)
...
<!-- - ⚖️ **Ethical considerations**: Technology choices have consequences -->
:::
...
class TSPVisualizer {
    ...
    async solveTSP() {
        ...
        if (this.cities.length > 10) {
            const shouldContinue = confirm(
                `⚠️ Warning: ${this.cities.length} cities will generate ${this.factorial(this.cities.length - 1) / 2} routes to check.\n\n` +
    ...
    updateInfo() {
        ...
        if (cityCount <= 3) {
            ...
            warningText = '⚠️ High complexity - will take several seconds to solve.';
        } else {
            warningText = '🚨 EXTREME complexity - may crash browser or take minutes!';
        }
    ...
    drawPlacementPreview() {
        ...
        if (!tooClose) {
            ...
            this.ctx.fillText('✗', this.mouseX, this.mouseY);
        }
